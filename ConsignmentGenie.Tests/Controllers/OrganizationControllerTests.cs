...
namespace ConsignmentGenie.Tests.Controllers
{
    public class OrganizationControllerTests : IDisposable
    {
        ...
        public OrganizationControllerTests()
        {
            ...
            // Setup mock configuration for CloudinaryPhotoService
            var mockConfiguration = new Mock<IConfiguration>();
            var mockCloudinarySection = new Mock<IConfigurationSection>();
            mockCloudinarySection.Setup(x => x["Url"]).Returns("cloudinary://123:secret@test");
            mockConfiguration.Setup(x => x.GetSection("Cloudinary")).Returns(mockCloudinarySection.Object);
            mockConfiguration.Setup(x => x["Cloudinary:Url"]).Returns("cloudinary://123:secret@test");

            var mockCloudinaryLogger = new Mock<ILogger<CloudinaryPhotoService>>();
            var cloudinaryService = new CloudinaryPhotoService(_context, mockConfiguration.Object, mockCloudinaryLogger.Object);

            _controller = new OrganizationController(_context, mockLogger.Object, cloudinaryService);
