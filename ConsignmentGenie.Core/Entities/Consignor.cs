...
public class Consignor : BaseEntity
{
    ...
    // Shop-owned inventory flag
    public bool IsShopOwned { get; set; } = false;  // True for system-generated shop-owned consignor
    ...
    // Contract Tracking
    // Note: ContractOnFile removed - computed from ConsignorAgreements table to avoid data duplication

    // Agreement tracking fields
    public DateTime? AgreementGeneratedAt { get; set; }

    [MaxLength(500)]
    public string? AgreementGeneratedUrl { get; set; }

    public DateTime? AcknowledgedTermsAt { get; set; }

    [MaxLength(45)]
    public string? AcknowledgedTermsIP { get; set; }
    ...
    public string? AgreementVerificationResult { get; set; }

    [MaxLength(20)]
    public string? AgreementStatus { get; set; } // not_required, pending, pending_upload, pending_physical, pending_review, approved, rejected

    public DateTime? AgreementReviewedAt { get; set; }

    public Guid? AgreementReviewedBy { get; set; }

    [MaxLength(500)]
    public string? AgreementReviewNote { get; set; }
