...
public class AdminController : ControllerBase
{
    ...
    private async Task CreateDemoDataAsync()
    {
        ...
        var testItems = new[]
        {
            new ConsignmentGenie.Core.Entities.Item
            {
                ...
                // TODO: Handle CategoryId for Electronics category
                Brand = "TestBrand",
            ...
            new ConsignmentGenie.Core.Entities.Item
            {
                ...
                Price = 45.50m,
                Status = ItemStatus.Available,
                // TODO: Handle CategoryId for Clothing category
                Brand = "TestFashion",
    ...
    public async Task<ActionResult<ApiResponse<object>>> GetMetrics()
    {
        try
        {
            ...
            var activeOrganizations = await _context.Organizations
                .CountAsync(o => o.CachedSubscriptionStatus == "active");
            var totalUsers = await _context.Users.CountAsync();

            // Calculate estimated revenue based on founder vs regular pricing
            var founderCount = await _context.Organizations
                .CountAsync(o => o.CachedSubscriptionStatus == "active" && o.IsFounderPricing);
            var regularCount = await _context.Organizations
                .CountAsync(o => o.CachedSubscriptionStatus == "active" && !o.IsFounderPricing);

            // Estimate monthly revenue (Founder average: $49, Regular: $49 base)
            var totalRevenue = (founderCount * 49) + (regularCount * 49);
        ...
        try
        {
            var recentSignups = await _context.Organizations
                ...
                .Select(o => new
                {
                    ...
                    subscriptionTier = o.IsFounderPricing ? "Founder" : "Standard",
                    subscriptionStatus = o.CachedSubscriptionStatus ?? "pending",
    ...
    /// <summary>
    /// Fix missing store slugs for existing organizations
    /// </summary>
    [HttpPost("fix-missing-slugs")]
    [Authorize(Roles = "Admin")]
    public async Task<ActionResult<ApiResponse<object>>> FixMissingSlugs()
    {
        try
        {
            _logger.LogInformation("Starting fix for missing organization slugs...");

            // Find organizations with missing or empty slugs
            var organizationsWithoutSlugs = await _context.Organizations
                .Where(o => o.Slug == null || o.Slug == "")
                .ToListAsync();

            if (!organizationsWithoutSlugs.Any())
            {
                return Ok(ApiResponse<object>.SuccessResult(new
                {
                    message = "All organizations already have slugs",
                    processedCount = 0
                }, "No slugs needed to be fixed"));
            }

            var processedCount = 0;
            var errors = new List<string>();

            foreach (var organization in organizationsWithoutSlugs)
            {
                try
                {
                    // Generate unique slug based on organization name
                    var slug = await _slugService.GenerateUniqueOrganizationSlugAsync(organization.Name, organization.Id);
                    ...
                    _logger.LogInformation("Generated slug '{Slug}' for organization '{OrganizationName}' (ID: {OrganizationId})",
                        slug, organization.Name, organization.Id);
                }
                catch (Exception ex)
                {
                    var error = $"Failed to generate slug for organization '{organization.Name}' (ID: {organization.Id}): {ex.Message}";
                    errors.Add(error);
                    _logger.LogError(ex, "Error generating slug for organization {OrganizationId}", organization.Id);
                }
            }
            ...
            _logger.LogInformation("Completed slug migration. Processed: {ProcessedCount}, Errors: {ErrorCount}",
                processedCount, errors.Count);

            return Ok(ApiResponse<object>.SuccessResult(new
            {
                processedCount,
                totalOrganizations = organizationsWithoutSlugs.Count,
                errors = errors.Any() ? errors : null
            }, $"Successfully generated {processedCount} slugs"));

        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error during slug migration: {Message}", ex.Message);
            return StatusCode(500, ApiResponse<object>.ErrorResult("Failed to fix missing slugs: " + ex.Message));
        }
    }
