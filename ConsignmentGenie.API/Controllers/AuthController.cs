...
public class AuthController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<LoginResponse>>> SetupProviderAccount([FromBody] ConsignorPortalSetupRequest request)
    {
        try
        {
            ...
            // Generate JWT token
            var token = _authService.GenerateJwtToken(user.Id, user.Email, user.Role.ToStringValue(), provider.OrganizationId);
    ...
    public async Task<ActionResult<ApiResponse<LoginResponse>>> LoginProvider([FromBody] ConsignorLoginRequest request)
    {
        try
        {
            ...
            // Generate JWT token
            var token = _authService.GenerateJwtToken(provider.User.Id, provider.User.Email, provider.User.Role.ToStringValue(), provider.OrganizationId);
