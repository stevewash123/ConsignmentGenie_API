...
public class MobileController : ControllerBase
{
    ...
    public async Task<IActionResult> ProcessQuickSale([FromBody] QuickSaleRequest request)
    {
        try
        {
            ...
            // Create transaction with new TransactionItem pattern
            var consignorSplit = item.Consignor.DefaultSplitPercentage ?? item.Consignor.CommissionRate;
            var consignorAmount = item.Price * (consignorSplit / 100);
            var shopAmount = item.Price - consignorAmount;

            var transaction = new ConsignmentGenie.Core.Entities.Transaction
            {
                OrganizationId = Guid.Parse(organizationId),
                TransactionDate = DateTime.UtcNow,
                PaymentType = request.PaymentMethod ?? "Cash",
                Subtotal = item.Price,
                TaxAmount = 0,
                TaxRate = 0,
                Total = item.Price,
            ...
            var transactionItem = new ConsignmentGenie.Core.Entities.TransactionItem
            {
                ...
                ConsignorSplitPercentage = consignorSplit / 100,
                ConsignorAmount = consignorAmount,
                StoreAmount = shopAmount
            };
            ...
            return Ok(new
            {
                ...
                data = new
                {
                    ...
                    providerAmount = consignorAmount,
                    shopOwnerAmount = shopAmount
                }
            });
        }
        catch (Exception ex)
        ...
        try
        {
            ...
            var recentSales = await _unitOfWork.Transactions
                .GetAllAsync(t => t.OrganizationId == Guid.Parse(organizationId),
                    includeProperties: "Items.Item,Items.Consignor");

            var salesData = recentSales
                .OrderByDescending(t => t.TransactionDate)
                .Take(limit)
                .SelectMany(sale => sale.Items.Select(item => new
                {
                    id = sale.Id,
        ...
        try
        {
            ...
            foreach (var saleData in request.OfflineSales)
            {
                try
                {
                    ...
                    // Create transaction item for batch sale
                    var transactionItem = new ConsignmentGenie.Core.Entities.TransactionItem
                    {
                        TransactionId = transaction.Id,
                        OrganizationId = Guid.Parse(organizationId),
                        ...
                        ConsignorSplitPercentage = saleData.ConsignorAmount / saleData.Amount,
                        ConsignorAmount = saleData.ConsignorAmount,
                        StoreAmount = saleData.ShopAmount
                    };
    ...
    private async Task<IActionResult> GetShopOwnerMobileDashboard(Guid organizationId)
    {
        ...
        var todaySales = await _unitOfWork.Transactions
            .CountAsync(t => t.OrganizationId == organizationId && t.TransactionDate.Date == today);

        var todayRevenue = (await _unitOfWork.Transactions
            .GetAllAsync(t => t.OrganizationId == organizationId && t.TransactionDate.Date == today))
            .Sum(t => t.Total);
