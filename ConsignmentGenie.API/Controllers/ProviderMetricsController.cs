...
public class ProviderMetricsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<ProviderMetricsDto>>> GetProviderMetrics(Guid id)
    {
        try
        {
            ...
            if (!providerExists)
            {
                return NotFound(ApiResponse<ProviderMetricsDto>.ErrorResult("Provider not found"));
            }
        ...
        {
            _logger.LogError(ex, "Error getting metrics for provider {ProviderId}", id);
    ...
    public async Task<ActionResult<ApiResponse<ProviderDashboardMetricsDto>>> GetProviderDashboard()
    {
        try
        {
            ...
            var activeProviders = await _context.Providers
                .Where(p => p.OrganizationId == organizationId && p.Status == ProviderStatus.Active)
                .CountAsync();
            ...
            var deactivatedProviders = await _context.Providers
                .Where(p => p.OrganizationId == organizationId && p.Status == ProviderStatus.Deactivated)
                .CountAsync();

            // Get providers with highest pending balances
            var topProvidersData = await _context.Providers
                .Where(p => p.OrganizationId == organizationId && p.Status == ProviderStatus.Active)
            ...
            var thisMonthNewProviders = await _context.Providers
                .Where(p => p.OrganizationId == organizationId && p.CreatedAt >= startOfThisMonth)
                .CountAsync();

            var lastMonthNewProviders = await _context.Providers
                .Where(p => p.OrganizationId == organizationId &&
        ...
        [FromQuery] int days = 30)
    {
        try
        {
            ...
            if (provider == null)
            {
                return NotFound(ApiResponse<ProviderActivityDto>.ErrorResult("Provider not found"));
            }
            ...
            var recentTransactions = await _context.Transactions
                .Include(t => t.Item)
                .Where(t => t.ProviderId == id && t.OrganizationId == organizationId && t.SaleDate >= cutoffDate)
            ...
            var recentItems = await _context.Items
                .Where(i => i.ProviderId == id && i.OrganizationId == organizationId && i.CreatedAt >= cutoffDate)
            ...
            var recentPayouts = await _context.Payouts
                .Where(p => p.ProviderId == id && p.OrganizationId == organizationId && p.CreatedAt >= cutoffDate)
        ...
        {
            _logger.LogError(ex, "Error getting activity for provider {ProviderId}", id);
