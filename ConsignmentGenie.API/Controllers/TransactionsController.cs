...
public class TransactionsController : ControllerBase
{
    ...
    private Guid GetOrganizationId()
    {
        ...
        _logger.LogInformation("Searching for organizationId claim, found: {Value}", orgIdClaim ?? "null");
        ...
        [FromBody] CreateTransactionRequest request)
    {
        try
        {
            ...
            var userNameClaim = User.FindFirst("name")?.Value ?? User.FindFirst("email")?.Value ?? "Unknown User";

            if (Guid.TryParse(userIdClaim, out var userId))
            {
                request.ProcessedByUserId = userId;
            }
            request.ProcessedByName = userNameClaim;
