...
public class StripeService : IStripeService
{
    ...
    public async Task<SubscriptionResult> UpdateSubscriptionAsync(Guid organizationId, SubscriptionTier newTier)
    {
        try
        {
            ...
            // Note: Organization tier is now managed through Stripe subscription items
            // The tier will be reflected in ActiveIntegrations through webhooks
            await _context.SaveChangesAsync();
    ...
    private async Task HandleSubscriptionCreatedAsync(Event stripeEvent)
    {
        ...
        if (organizationId.HasValue)
        {
            ...
            if (organization != null)
            {
                // Update subscription IDs and cached status
                organization.StripeSubscriptionId = subscription.Id;
                organization.CachedSubscriptionStatus = subscription.Status;
                organization.CachedPeriodEnd = subscription.CurrentPeriodEnd;

                // Set founder pricing from metadata if this is the initial setup
                if (subscription.Metadata.TryGetValue("is_founder", out var isFounderStr) && bool.TryParse(isFounderStr, out var isFounder))
                {
                    organization.IsFounderPricing = isFounder;
                }

                if (subscription.Metadata.TryGetValue("founder_tier", out var tierStr) && int.TryParse(tierStr, out var tier))
                {
                    organization.FounderTier = tier;
                }

                await _context.SaveChangesAsync();

                _logger.LogInformation("Created subscription for organization {OrganizationId}: SubscriptionId={SubscriptionId}, Status={Status}",
                    organizationId, subscription.Id, subscription.Status);
            }
        }
    }
    ...
    private async Task HandleSubscriptionUpdatedAsync(Event stripeEvent)
    {
        ...
        if (organizationId.HasValue)
        {
            ...
            if (organization != null)
            {
                // Update cached subscription status and period end
                organization.CachedSubscriptionStatus = subscription.Status;
                organization.CachedPeriodEnd = subscription.CurrentPeriodEnd;

                // Extract current integrations from line items
                var integrations = subscription.Items.Data
                    .Select(item => item.Price.LookupKey)
                    .Where(key => key != null && !key.StartsWith("base_"))
                    .Select(key => ExtractFeatureName(key))
                    .ToArray();

                organization.ActiveIntegrations = integrations;

                await _context.SaveChangesAsync();

                _logger.LogInformation("Updated subscription for organization {OrganizationId}: Status={Status}, Integrations={Integrations}",
                    organizationId, subscription.Status, string.Join(",", integrations));
            }
        }
    }
    ...
    private async Task HandleSubscriptionDeletedAsync(Event stripeEvent)
    {
        ...
        if (organizationId.HasValue)
        {
            ...
            if (organization != null)
            {
                organization.CachedSubscriptionStatus = "canceled";
                await _context.SaveChangesAsync();
            }
        }
    }
    ...
    private async Task HandlePaymentSucceededAsync(Event stripeEvent)
    {
        ...
        if (!string.IsNullOrEmpty(subscriptionId))
        {
            ...
            if (organization != null)
            {
                organization.CachedSubscriptionStatus = "active";
                await _context.SaveChangesAsync();
            }
        }
    }
    ...
    private async Task HandlePaymentFailedAsync(Event stripeEvent)
    {
        ...
        if (!string.IsNullOrEmpty(subscriptionId))
        {
            ...
            if (organization != null)
            {
                organization.CachedSubscriptionStatus = "past_due";
                await _context.SaveChangesAsync();

                // Send notification to owner about subscription payment failure
                try
                {
                    var owner = await _context.Users
                        .FirstOrDefaultAsync(u => u.OrganizationId == organization.Id && u.Role == UserRole.Owner);

                    if (owner != null)
                    {
                        var failureReason = invoice?.Charge?.FailureMessage ?? "Payment failed";
                        var notification = NotificationHelper.CreateSubscriptionFailedNotification(
                            owner, organization.Id, failureReason);

                        await _notificationService.CreateNotificationAsync(notification);
                        _logger.LogInformation("Subscription payment failed notification sent to owner {OwnerEmail}", owner.Email);
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to send subscription payment failed notification for organization {OrganizationId}", organization.Id);
                }
            }
        }
    }
    ...
    public async Task<IntegrationPricingResult> GetIntegrationPricingAsync(Guid organizationId, string integration)
    {
        try
        {
            var organization = await _context.Organizations.FindAsync(organizationId);
            if (organization == null)
            {
                return new IntegrationPricingResult { Success = false, ErrorMessage = "Organization not found" };
            }

            // Determine if user has founder pricing
            var isFounder = organization.IsFounderPricing;

            // Get the appropriate product lookup key
            var lookupKey = integration.ToLower() switch
            {
                "quickbooks" => isFounder ? "quickbooks_founder" : "quickbooks_regular",
                "square" => isFounder ? "square_founder" : "square_regular",
                "dwolla" => isFounder ? "dwolla_founder" : "dwolla_regular",
                _ => null
            };

            if (string.IsNullOrEmpty(lookupKey))
            {
                return new IntegrationPricingResult { Success = false, ErrorMessage = $"Unknown integration: {integration}" };
            }

            // Query Stripe for the product price
            ...
            if (!prices.Data.Any())
            {
                return new IntegrationPricingResult { Success = false, ErrorMessage = $"Price not found for {lookupKey}" };
            }

            var price = prices.Data.First();
            var monthlyAmount = price.UnitAmount ?? 0;
            ...
            return new IntegrationPricingResult
            {
                Success = true,
                ...
                IsFounderPricing = isFounder
            };
        }
        catch (Stripe.StripeException ex)
        {
            _logger.LogError(ex, "Stripe error getting integration pricing for {Integration}, Organization {OrganizationId}", integration, organizationId);
            return new IntegrationPricingResult { Success = false, ErrorMessage = ex.Message };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting integration pricing for {Integration}, Organization {OrganizationId}", integration, organizationId);
            return new IntegrationPricingResult { Success = false, ErrorMessage = ex.Message };
        }
    }
