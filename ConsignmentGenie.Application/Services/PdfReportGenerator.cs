...
public class PdfReportGenerator : IPdfReportGenerator
{
    ...
    public async Task<byte[]> GenerateTextDocumentPdfAsync(string textContent, string title = "Document")
    {
            ...
            container.Page(page =>
            {
                ...
                page.Content()
                    ...
                    .Column(column =>
                    {
                        ...
                        // Split text into lines and process for better formatting
                        ...
                        for (int i = 0; i < lines.Length; i++)
                        {
                            ...
                            // Process address formatting - replace line breaks with commas in addresses
                            if (i > 0 && lines[i-1].Contains("located at") && !trimmedLine.Contains("TERMS"))
                            {
                                // This looks like an address continuation, format it properly
                                var addressLines = new List<string>();
                                var j = i;
                                while (j < lines.Length && !string.IsNullOrWhiteSpace(lines[j]) && !lines[j].Contains("TERMS"))
                                {
                                    addressLines.Add(lines[j].Trim());
                                    j++;
                                }

                                if (addressLines.Count > 0)
                                {
                                    var formattedAddress = string.Join(", ", addressLines);
                                    column.Item().Text(formattedAddress)
                                        .FontSize(11)
                                        .LineHeight(1.4f);

                                    // Add extra spacing after address
                                    column.Item().PaddingTop(8);

                                    i = j - 1; // Skip the processed address lines
                            ...
                            bool isHeader = (trimmedLine.Length < 100 && trimmedLine.ToUpper() == trimmedLine &&
                                           (trimmedLine.Contains("TERMS") || trimmedLine.Contains("AGREEMENT") ||
                                            trimmedLine.Contains("SIGNATURE") || trimmedLine.EndsWith(":"))) ||
                                           trimmedLine.StartsWith("CONSIGNMENT AGREEMENT");
                            ...
                            if (lastLineWasEmpty ||
                                (i > 0 && (lines[i-1].Contains("email") || lines[i-1].Contains("@"))) ||
                                lastLineWasHeader)
                            {
                                column.Item().PaddingTop(8);
                            }
                ...
                page.Footer()
                    ...
                    {
                        x.Span("Generated on ");
                        x.Span($"{DateTime.Now:MMMM d, yyyy}").SemiBold();
                    });
            });
        }).GeneratePdf());
    }
