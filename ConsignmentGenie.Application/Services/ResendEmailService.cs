...
<body style=""font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;"">
    ...
    <div style=""background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #ddd;"">
        ...
        <ol style=""font-size: 14px; color: #666; margin: 0; padding-left: 20px;"">
            ...
            <li>Complete your consignor registration</li>
        ...
        {
            _logger.LogError(ex, "[EMAIL] Failed to send consignor invitation email to {Email} for consignor {ConsignorName} from shop {ShopName}", email, consignorName, shopName);
            return false;
        }
    }

    public async Task<bool> SendConsignorWelcomeAsync(string email, string consignorName, string shopName, string storeCode, string ownerName)
    {
        _logger.LogInformation("[EMAIL] Starting consignor welcome email send to {Email} for consignor {ConsignorName} at shop {ShopName}", email, consignorName, shopName);

        try
        {
            var subject = $"Welcome to {shopName}!";
...
<head>
    ...
    <style>
        ...
        .cta-button {{ background-color: #4F46E5; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block; margin: 20px 0; }}
    </style>
</head>
<body>
    <div class=""container"">
        ...
        <div class=""content"">
            <h2>Hi {consignorName},</h2>
            <p>Welcome to {shopName}! {ownerName} has added you as a consignor.</p>

            <p><strong>Your shop details:</strong></p>
            <ul>
                <li><strong>Shop:</strong> {shopName}</li>
                <li><strong>Store Code:</strong> {storeCode}</li>
                <li><strong>Contact:</strong> {ownerName}</li>
            </ul>

            <p>You can now start consigning items at {shopName}. If you'd like to access your consignor portal online, you can create an account using this store code.</p>

            <p>If you have any questions, please don't hesitate to reach out to {ownerName} or the shop directly.</p>

            <p>Thank you for choosing to consign with {shopName}!</p>

            <p>Best regards,<br>The {shopName} Team</p>
        </div>
        <div class=""footer"">
            <p>This email was sent by {shopName} using ConsignmentGenie</p>
        </div>
    </div>
</body>
</html>";
            ...
            var result = await SendSimpleEmailAsync(email, subject, htmlContent);
            _logger.LogInformation("[EMAIL] Consignor welcome email send result for {Email}: {Result}", email, result);
            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[EMAIL] Failed to send consignor welcome email to {Email} for consignor {ConsignorName} at shop {ShopName}", email, consignorName, shopName);
            return false;
        }
    }
