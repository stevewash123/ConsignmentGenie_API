...
public class ReportsService : IReportsService
{
    ...
    public async Task<ServiceResult<SalesReportDto>> GetSalesReportAsync(Guid organizationId, SalesReportFilterDto filter)
    {
        try
        {
            ...
            // Generate line items (now one line per transaction item)
            var lineItems = transactionList
                .SelectMany(t => t.Items.Select(ti => new SalesLineItemDto
                {
                    TransactionId = t.Id,
                    Date = t.TransactionDate,
                    ItemName = ti.Item.Title,
                    Category = ti.Item.ItemCategoryNavigation?.Name ?? "",
                    ConsignorName = ti.Consignor.DisplayName ?? "",
                    SalePrice = ti.UnitPrice * ti.Quantity,
                    ShopCut = ti.StoreAmount,
                    ConsignorCut = ti.ConsignorAmount,
            ...
            foreach (var provider in providerList)
            {
                ...
                if (transactions.Any())
                {
                    var daysToSell = transactions
                        .SelectMany(t => t.Items.Where(ti => ti.Item.ListedDate.HasValue))
                        .Select(ti => (ti.Transaction.TransactionDate - ti.Item.ListedDate!.Value.ToDateTime(TimeOnly.MinValue)).TotalDays)
                        .ToList();
        ...
        try
        {
            var itemsQuery = await _unitOfWork.Items
                .GetAllAsync(i => i.OrganizationId == organizationId &&
                    ...
                    includeProperties: "Consignor,ItemCategoryNavigation");
            ...
            if (filter.Categories != null && filter.Categories.Any())
            {
                items = items.Where(i => filter.Categories.Contains(i.ItemCategoryNavigation != null ? i.ItemCategoryNavigation.Name : ""));
            }
            ...
            var agingItemDtos = agingItems
                ...
                .Select(x => new AgingItemDto
                {
                    ...
                    Name = x.Item.Title,
                    SKU = x.Item.Sku,
                    Category = x.Item.ItemCategoryNavigation?.Name ?? "",
    ...
    public async Task<ServiceResult<PayoutSummaryReportDto>> GetPayoutSummaryReportAsync(Guid organizationId, PayoutSummaryFilterDto filter)
    {
        try
        {
            ...
            var providerSummaries = transactionList
                ...
                .Select(g =>
                {
                    var consignorItems = g.ToList();
                    var totalSales = consignorItems.Sum(x => x.TransactionItem.UnitPrice * x.TransactionItem.Quantity);
                    var providerCut = consignorItems.Sum(x => x.TransactionItem.ConsignorAmount);
                    var alreadyPaid = consignorItems
                        .Where(x => x.Transaction.ConsignorPaidOut)
                        .Sum(x => x.TransactionItem.ConsignorAmount);
                    var pendingBalance = consignorItems
                        .Where(x => !x.Transaction.ConsignorPaidOut)
                        .Sum(x => x.TransactionItem.ConsignorAmount);

                    var lastPayoutDate = consignorItems
    ...
    public async Task<ServiceResult<DailyReconciliationDto>> GetDailyReconciliationReportAsync(Guid organizationId, DateOnly date)
    {
        try
        {
            ...
            var cashSales = transactionList.Where(t => t.PaymentType == "Cash").Sum(t => t.Total);
            var cardSales = transactionList.Where(t => t.PaymentType == "Card").Sum(t => t.Total);
            var checkSales = transactionList.Where(t => t.PaymentType == "Check").Sum(t => t.Total);
            var otherSales = transactionList.Where(t => !new[] { "Cash", "Card", "Check" }.Contains(t.PaymentType)).Sum(t => t.Total);
            var totalSales = transactionList.Sum(t => t.Total);
    ...
    public async Task<ServiceResult<TrendsReportDto>> GetTrendsReportAsync(Guid organizationId, TrendsFilterDto filter)
    {
        try
        {
            ...
            var categoryTrends = transactionList
                .SelectMany(t => t.Items.Where(ti => !string.IsNullOrEmpty(ti.Item.ItemCategoryNavigation?.Name)))
    ...
    public async Task<ServiceResult<InventoryOverviewDto>> GetInventoryOverviewAsync(Guid organizationId)
    {
        try
        {
            var items = await _unitOfWork.Items
                .GetAllAsync(i => i.OrganizationId == organizationId, includeProperties: "Consignor,ItemCategoryNavigation");

            var itemList = items.ToList();

            var categoryBreakdown = itemList
                .GroupBy(i => i.ItemCategoryNavigation?.Name ?? "Uncategorized")
    ...
    public async Task<ServiceResult<byte[]>> ExportConsignorPerformanceReportAsync(Guid organizationId, ConsignorPerformanceFilterDto filter, string format)
    {
        try
        {
            var reportResult = await GetConsignorPerformanceReportAsync(organizationId, filter);
            if (!reportResult.Success)
                return ServiceResult<byte[]>.FailureResult(reportResult.Message, reportResult.Errors);
            ...
            {
                var pdf = GenerateConsignorPerformancePdf(data, filter);
                return ServiceResult<byte[]>.SuccessResult(pdf);
            }
        ...
        {
            _logger.LogError(ex, "Error exporting consignor performance report for organization {OrganizationId}", organizationId);
            return ServiceResult<byte[]>.FailureResult("Failed to export consignor performance report", new List<string> { ex.Message });
        }
    }
    ...
    private static byte[] GenerateConsignorPerformancePdf(ConsignorPerformanceReportDto data, ConsignorPerformanceFilterDto filter)
    {
        return Document.Create(container =>
        {
            container.Page(page =>
