            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    if (adminUserId.HasValue)
                    {
                        ...
                        // Create notification request using Core DTOs
                        var createRequest = new Core.DTOs.Notifications.CreateNotificationRequest
                        {
                            FromUserId = user.Id, // From the user submitting the suggestion
                            FromType = "customer", // Assuming this comes from customers/users
                            ToUserId = notificationRequest.UserId, // To the recipient (likely owner)
                            ToType = "owner", // Default role for suggestion notifications
                            Type = notificationRequest.Type.ToString(),
                            Title = "New Suggestion Submitted",
                            Message = suggestion.Message,
                            OrganizationId = user.OrganizationId
                        };

                        var notification = await _notificationService.CreateAsync(createRequest);
                        var success = notification != null;
