...
public class CloudinaryPhotoService : IPhotoService
{
    ...
    public async Task<(string Url, int Width, int Height)> UploadLogoAsync(Guid organizationId, Stream fileStream, string fileName)
    {
        try
        {
            ...
            var uploadParams = new ImageUploadParams()
            {
                ...
                Transformation = new Transformation()
                    .Quality("auto:good")
                    .FetchFormat("auto"),
                Folder = $"consignment/logos",
                Tags = "consignment,logo," + organizationId.ToString()
            };
            ...
            _logger.LogInformation("Logo uploaded successfully to Cloudinary: {LogoUrl}", logoUrl);

            // Return URL with dimensions
            return (logoUrl, uploadResult.Width, uploadResult.Height);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to upload logo for organization {OrganizationId}", organizationId);
            throw;
        }
    }

    public async Task<bool> DeleteLogoAsync(string logoUrl)
    {
        try
        {
            // Extract public ID from Cloudinary URL
            ...
            if (success)
            {
                _logger.LogInformation("Logo deleted successfully from Cloudinary: {LogoUrl}", logoUrl);
            }
            else
            {
                _logger.LogWarning("Failed to delete logo from Cloudinary: {LogoUrl}, Result: {Result}", logoUrl, result.Result);
            }
        ...
        {
            _logger.LogError(ex, "Failed to delete logo {LogoUrl}", logoUrl);
            return false;
        }
    }
