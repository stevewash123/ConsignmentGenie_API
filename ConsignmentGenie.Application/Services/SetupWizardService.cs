...
public class SetupWizardService : ISetupWizardService
{
    ...
    public async Task<SetupCompleteDto> CompleteSetupAsync(Guid organizationId, bool startTrial = true, string? subscriptionPlan = null)
    {
        ...
        // Note: Trial management is now handled by Stripe during registration
        // SetupWizardService no longer starts trials directly
        organization.UpdatedAt = DateTime.UtcNow;
        ...
        // Get trial info from current subscription status
        var isTrialing = organization.CachedSubscriptionStatus == "trialing";
        var daysRemaining = 0;
        if (isTrialing && organization.CachedPeriodEnd.HasValue)
        {
            daysRemaining = Math.Max(0, (int)(organization.CachedPeriodEnd.Value - DateTime.UtcNow).TotalDays);
        }

        var trialInfo = new TrialSubscriptionDto
        {
            Status = organization.CachedSubscriptionStatus ?? "pending",
            TrialStartedAt = null, // Would need to get from Stripe if needed
            TrialEndsAt = organization.CachedPeriodEnd,
            DaysRemaining = daysRemaining,
            TrialExtensionsUsed = 0, // No longer supported - managed by Stripe
            CanExtendTrial = false, // Managed by Stripe
            SubscriptionTier = SubscriptionTier.Basic, // Default tier for all subscriptions in new system
            SubscriptionPlan = organization.IsFounderPricing ? "Founder" : "Standard",
            StripeSubscriptionStatus = organization.CachedSubscriptionStatus,
            SubscriptionStartedAt = null, // Would need to get from Stripe if needed
            CurrentPeriodEnd = organization.CachedPeriodEnd
        };
    ...
    private async Task<object> GetCompleteSetupDataAsync(Organization organization)
    {
        ...
        return new
        {
            ...
            {
                TrialLength = 14,
                CanStartTrial = organization.CachedSubscriptionStatus == null || organization.CachedSubscriptionStatus == "pending"
            }
        };
    }
}