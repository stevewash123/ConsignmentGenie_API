            ...
            foreach (var consignor in consignorList)
            {
                ...
                if (transactions.Any())
                {
                    var daysToSell = transactions
                        ...
                        .Where(x => x.TransactionItem.Item.ListedDate.HasValue)
                        .Select(x => (x.Transaction.TransactionDate - x.TransactionItem.Item.ListedDate!.Value.ToDateTime(TimeOnly.MinValue)).TotalDays)
                        .ToList();
