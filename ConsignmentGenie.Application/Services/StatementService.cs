...
public class StatementService : IStatementService
{
    ...
    public async Task<StatementDto> GenerateStatementAsync(Guid consignorId, DateOnly periodStart, DateOnly periodEnd)
    {
        ...
        var totalEarnings = transactions.SelectMany(t => t.Items.Where(ti => ti.ConsignorId == consignorId)).Sum(ti => ti.ConsignorAmount);
        ...
        _logger.LogInformation("Generated statement {StatementNumber} for consignor {ConsignorId}", statementNumber, consignorId);
    ...
    public async Task GenerateStatementsForMonthAsync(int year, int month)
    {
        ...
        foreach (var consignorId in consignors)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Failed to generate statement for consignor {ConsignorId} for period {Year}-{Month}", consignorId, year, month);
            }
        }

        _logger.LogInformation("Completed statement generation for {Year}-{Month}. Processed {ConsignorCount} consignors", year, month, consignors.Count());
    }
    ...
    private async Task<StatementDto> MapToStatementDto(Statement statement)
    {
        ...
        var sales = await _context.Transactions
            .Include(t => t.Items)
                .ThenInclude(ti => ti.Item)
            .Where(t => t.Items.Any(ti => ti.ConsignorId == statement.ConsignorId)
            ...
            .SelectMany(t => t.Items.Where(ti => ti.ConsignorId == statement.ConsignorId)
                .Select(ti => new StatementSaleLineDto
                {
                    Date = t.TransactionDate,
                    ItemSku = ti.Item.Sku ?? "",
                    ItemTitle = ti.Item.Title,
                    SalePrice = ti.UnitPrice * ti.Quantity,
                    CommissionRate = ti.ConsignorSplitPercentage,
                    EarningsAmount = ti.ConsignorAmount
                }))
            .ToListAsync();
