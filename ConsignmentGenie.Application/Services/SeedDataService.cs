...
public class SeedDataService
{
    ...
    private async Task<List<Item>> CreateItems(Guid orgId, List<Consignor> providers)
    {
        ...
        for (int i = 0; i < Math.Min(38, itemTemplates.Length); i++)
        {
            ...
            var item = new Item
            {
                ...
                // CategoryId = null, // Optional - can be set later
                Status = status,
    ...
    private async Task<List<Transaction>> CreateTransactions(Guid orgId, List<Item> items)
    {
        ...
        foreach (var item in soldItems)
        {
            ...
            var transaction = new Transaction
            {
                ...
                PaymentType = paymentMethods[random.Next(paymentMethods.Length)],
                Subtotal = salePrice,
                TaxAmount = salePrice * 0.0875m, // 8.75% sales tax
                Total = salePrice + (salePrice * 0.0875m),
    ...
    private async Task CreateHistoricalPayouts(Guid orgId, List<Consignor> providers, List<Transaction> transactions)
    {
        ...
        var oldTransactions = transactions
            .Where(t => t.TransactionDate < DateTime.UtcNow.AddDays(-35))
        ...
        foreach (var providerGroup in oldTransactions)
        {
            ...
            foreach (var transactionItem in providerGroup)
            {
                transactionItem.Transaction.ConsignorPaidOut = true;
                transactionItem.Transaction.ConsignorPaidOutDate = payoutDate;
                transactionItem.Transaction.PayoutMethod = provider.PreferredPaymentMethod;
                transactionItem.Transaction.PayoutNotes = $"Monthly payout - {payoutDate:MMM yyyy}";
            }
        }
